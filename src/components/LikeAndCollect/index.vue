...
import {
  praiseBlogByUid, getBlogPraiseCountByUid, CollectRecipe, reportBlog
...
export default {
  ...
  methods: {
    ...
    collectToggle: function () {
      //this.$commonUtil.message.info('收藏成功')
    ...
    //   })
    // },
    collectRecipe: function (uid) {
      let that = this
      var params = new URLSearchParams()
      params.append('uid', uid)
      //params.append('collectName', collectName)
      //console.log('111111 ' + collectName)
      this.$commonUtil.message.info('收藏')
      CollectRecipe(params).then(response => {
        if (response.data.code === this.$ECode.SUCCESS) {
          //that.collectToggle()
